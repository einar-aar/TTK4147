#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <gpio.h>
#include <power_clocks_lib.h>
#include <sysclk.h>
#include <compiler.h>
#include <board.h>
#include <stdio_usb.h>


#define CONFIG_USART_IF (AVR32_USART2)

#include "FreeRTOS.h"
#include "task.h"

#define TEST_A      AVR32_PIN_PA31
#define RESPONSE_A  AVR32_PIN_PA30
#define TEST_B      AVR32_PIN_PA29
#define RESPONSE_B  AVR32_PIN_PA28
#define TEST_C      AVR32_PIN_PA27
#define RESPONSE_C  AVR32_PIN_PB00


void busy_delay_ms(int delay){
    for(; delay != 0; delay--){
        for(int i = 0; i < 2108; i++){
            asm volatile ("" ::: "memory");
        }
    }
}

void busy_delay_short(void){
    for(int i = 0; i < 10; i++){
        asm volatile ("" ::: "memory");
    }
}

void init(){
	board_init();
	
    gpio_configure_pin(TEST_A, GPIO_DIR_INPUT);
    gpio_configure_pin(TEST_B, GPIO_DIR_INPUT);
    gpio_configure_pin(TEST_C, GPIO_DIR_INPUT);
    gpio_configure_pin(RESPONSE_A, GPIO_DIR_OUTPUT | GPIO_INIT_HIGH);
    gpio_configure_pin(RESPONSE_B, GPIO_DIR_OUTPUT | GPIO_INIT_HIGH);
    gpio_configure_pin(RESPONSE_C, GPIO_DIR_OUTPUT | GPIO_INIT_HIGH);

	pcl_switch_to_osc(PCL_OSC0, FOSC0, OSC0_STARTUP);
	
	stdio_usb_init(&CONFIG_USART_IF);

    #if defined(__GNUC__) && defined(__AVR32__)
	    setbuf(stdout, NULL);
	    setbuf(stdin,  NULL);
    #endif
}

// Tasks
static void task_A(void* args){

	while(1) {

        if (!gpio_get_pin_value(TEST_A)) {
            
            gpio_clr_gpio_pin(RESPONSE_A);
            busy_delay_short();
            gpio_set_gpio_pin(RESPONSE_A);

        } else {

            vTaskDelay(1);
        }
	}
}

static void task_B(void* args){

	while(1) {

        if (!gpio_get_pin_value(TEST_B)) {
            
            gpio_clr_gpio_pin(RESPONSE_B);
            busy_delay_short();
            gpio_set_gpio_pin(RESPONSE_B);

        } else {

            vTaskDelay(1);
        }
	}
}

static void task_C(void* args){

	while(1){

        if (!gpio_get_pin_value(TEST_C)) {
            
            busy_delay_ms(3);
            gpio_clr_gpio_pin(RESPONSE_C);
            busy_delay_short();
            gpio_set_gpio_pin(RESPONSE_C);
            
        } else {

            vTaskDelay(1);
        }
	}
}


int main() {
    
	init();
        
	xTaskCreate(task_A, "", 1024, NULL, tskIDLE_PRIORITY + 2, NULL);

    xTaskCreate(task_B, "", 1024, NULL, tskIDLE_PRIORITY + 2, NULL);

    xTaskCreate(task_C, "", 1024, NULL, tskIDLE_PRIORITY + 1, NULL);

	// Start the scheduler, anything after this will not run.
	vTaskStartScheduler();
    
}
